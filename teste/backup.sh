...
if [ -z "$DATABASE_ENTRIES" ]; then
  echo "❌ Erro: DATABASE_ENTRIES não foi definido."
  exit 1
fi

mkdir -p "$TMP_DIR"

# Espera DATABASE_ENTRIES no formato:
...
for entry in "${DATABASES[@]}"; do
  ...
  echo "🔄 Recriando banco $DST_DB_NAME no destino ($DST_DB_HOST:$DST_DB_PORT)..."
...
tar -czf "$TMP_DIR/$ARCHIVE_NAME" -C "$TMP_DIR" ./*.sql
...
# Copiar para destino final (se definido)
if [ -n "$BACKUP_DEST_PATH" ]; then
  echo "📁 Movendo backup para $BACKUP_DEST_PATH"
  mkdir -p "$BACKUP_DEST_PATH"
  mv "$TMP_DIR/$ARCHIVE_NAME" "$BACKUP_DEST_PATH/"
fi
